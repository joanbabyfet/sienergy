...
class ctl_admin_user_login extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //獲取數據
        $rows = mod_admin_user_login::list_data([
            'username'  =>  $username,
            'date1'      =>  $date1,
            'date2'      =>  $date2,
        ...
        return view('admin.admin_user_login_index', [
            'list'  =>  $rows['data'],
            'pages' =>  $pages,
        ]);
    }
    ...
    public function delete(Request $request)
    {
        ...
        $status = mod_admin_user_login::del_data([
            '_id'           => $id + [-1],
        ]);
        if($status < 0)
        {
            return mod_common::error(mod_admin_user_login::get_err_msg($status), $status);
        }
        //寫入日志
        mod_admin_user_oplog::add_log("用戶登入日志刪除 ".implode(",", $id));
