...
class mod_currency_type extends mod_model
{
    //主键
    ...
    protected function list_data(array $conds)
    {
        ...
        $rows = self::get_list([
            'fields'    => ['id', 'name', 'code', 'symbol', 'sort', 'status', 'create_time'
            ],
            'where'     => $where,
            'page'      => $page,
            'page_size' => $page_size,
    ...
    //获取币种选项
    protected function get_currencys()
    {
        $rows = self::get_list([
            'where'     =>  [
                ['status', '=', self::ENABLE],
        ...
        $ret = mod_array::one_array($rows, ['code', 'name']);
        return $ret;
    }
}
