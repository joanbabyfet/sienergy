    ...
    <div class="layui-card-body ">
        <script type="text/html" id="toolbar">
            <div class="layui-btn-container">
                <a class="layui-btn layui-btn-sm" onclick="admin.openLayerForm('{{ route("admin.faq_cat.add") }}', '問答分類添加', 'POST', '500px', '350px')"><i class="layui-icon"></i>新增</a>
                ...
                <tr>
                    ...
                    <td>
                        <a class="layui-btn layui-btn-xs"
                           onclick="admin.openLayerForm('{{ route("admin.faq_cat.edit").'?id='.$v['id'] }}', '問答分類編輯', 'POST', '500px', '350px')">編輯</a>
                        <a class="layui-btn layui-btn-xs layui-btn-danger"
                           onclick="admin.tableDataDelete('{{ route("admin.faq_cat.delete").'?ids[]='.$v['id'] }}', this)">刪除</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
            ...
            table.on('toolbar(list)', function(obj){
                ...
                switch(obj.event)
                {
                    case 'batch_del': //批量刪除
                        if(flag !== 0){
                            layui.layer.confirm('你確認刪除嗎？', {icon: 3, title: "提示", btn: ['確定', '取消']}, function (){
                                $.ajax({
                                    url: '{{ route('admin.faq_cat.delete') }}',
                                    type: 'POST',
                    ...
                    case 'batch_enable': //批量啟用
                        if(flag !== 0){
                            layui.layer.confirm('你確認啟用嗎？', {icon: 3, title: "提示", btn: ['確定', '取消']}, function (){
                                $.ajax({
                                    url: '{{ route('admin.faq_cat.enable') }}',
                                    type: 'POST',
                                    data: { ids: ids }, //送到服务器數據
                                    success: function(res) {
                                        if (res.code === 0) {
                    ...
                    case 'batch_disable': //批量禁用
                        if(flag !== 0){
                            layui.layer.confirm('你確認禁用嗎？', {icon: 3, title: "提示", btn: ['確定', '取消']}, function (){
                                $.ajax({
                                    url: '{{ route('admin.faq_cat.disable') }}',
                                    type: 'POST',
                                    data: { ids: ids }, //送到服务器數據
                                    success: function(res) {
                                        if (res.code === 0) {
                    ...
                    case 'export':
                        ...
                        export_excel('{{ route('admin.faq_cat.export_list') }}', _data, search_str);
                        break;
                }
            });
