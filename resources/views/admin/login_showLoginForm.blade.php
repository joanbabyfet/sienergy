    ...
    <form class="layui-form" lay-filter="form-box" action="" method="POST">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="username" required  lay-verify="required" lay-verType="" placeholder="請輸入用戶名" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
...
<script>
    ...
    layui.use('form', function(){
        ...
        {{--form.on('submit(login)', function(data){--}}
        {{--    $.post("{{ route("admin.login.login") }}", data.field, function(response) {--}}
        {{--        if (response.code === 0) {--}}
        {{--            layui.layer.msg(response.msg, {time: 2000, icon: 6});--}}
        {{--            window.location = "{{ route("admin.index.index") }}";--}}
        {{--        } else {--}}
